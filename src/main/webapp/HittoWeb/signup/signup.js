document.addEventListener('DOMContentLoaded', function() {
    ...
    if (userId === '' || userId.length < 4) {
        userIdError.textContent = '아이디를 4자 이상 입력해주세요.';
        return;
    }

		fetch('/JSLIM/HittoWeb/signup/checkId.jsp', {
		    method: 'POST',
		    headers: {
		...
		.then(data => {
		    const result = data.trim().toLowerCase();  // 🔥 개행 제거 + 소문자 통일
		    console.log("서버 응답 정리된 결과:", result);
...
// 아이디 입력 필드가 변경되면 중복 확인 초기화
    ...
    signupForm.addEventListener('submit', function(e) {
        ...
        if (isValid) {
            ...
            // 회원가입 요청 전송
			fetch('/JSLIM/HittoWeb/joinProc.jsp', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
			...
			.then(data => {
			    ...
			    if (result === 'success') {
			        alert('🎉 회원가입 완료!\n\nHittoStore에 오신 걸 환영합니다 😊');
			        window.location.href = '../index.jsp';
			    } else if (result === 'duplicate') {
			        alert('이미 존재하는 아이디입니다.');
			    } else {
			        alert('회원가입에 실패했습니다. ' + result);
			    }
			})
