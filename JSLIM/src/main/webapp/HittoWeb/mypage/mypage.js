...
function openLoginModal() {
  ...
  fetch('../components/login-modal.jsp')
    .then(response => {
      if (!response.ok) {
...
function checkLoginStatus() {
  ...
  if (isLoggedIn && loginLink && userNameLink) {
    ...
    if (userId) {
      ...
      userNameLink.setAttribute('href', './mypage.jsp');
    }
  }
...
function initEventListeners() {
  ...
  if (editProfileBtn) {
    editProfileBtn.addEventListener('click', function() {
      ...
      // window.location.href = 'edit-profile.jsp'; // 실제 구현시 주석 해제
    });
  }
  ...
  cancelOrderBtns.forEach(btn => {
    if (btn.textContent === '주문취소') {
      ...
      btn.addEventListener('click', function() {
        ...
        // window.location.href = 'delivery-tracking.jsp'; // 실제 구현시 주석 해제
      });
    }
  });
